...
import { addAdminInitialValue, addAdminSchema, genders, updateAdminSchema } from '../Schema'
...
const Form = (props) => {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        if (formIsOpen) {
            ...
            let data = await updateAdminApi(values);
            if (data?.status === 200) {
                setSnackbarAlertOpen(true);
                setSnackbarContent({
                    type: 'success',
    ...
    const formik = useFormik({
        ...
        validationSchema: formIsOpen ? addAdminSchema : updateAdminSchema,
    ...
    return (
        <>
              <Modal open={props?.open} onCancel={handleClose} footer={null}>
                <p className='text-xl font-semibold mt-3 mb-3'> {formIsOpen ? 'Add Admin' : 'Update Admin'} </p>
